...
import {uploadFile, requestGPT} from '@/app/services';
...
export default function Page() {
  ...
  const handleUpload = async () => {
    ...
    uploadFile(form)
      ...
      .then(data => {
        if (data.code === 0) {
          setCurrentImageUrl(data.data.url);
        } else {
          setUploadError(true);
        }
      });
        ...
        <div className="col-span-8 lg:col-span-5 2xl:col-span-3 bg-zinc-900 rounded-lg p-6">
          <div className="flex items-start h-[calc(100dvh-8rem)] lg:h-full">
            ...
            <div className="ml-4 flex flex-col flex-1 h-full">
                      ...
                      {isDragActive ? (
                        ...
                        <>
                          ...
                          <p className="text-xs text-gray-500 dark:text-gray-400">
                            仅 <span className="font-semibold">GPT-4o</span> 支持图片输入，支持 JPG、PNG、WEBP 格式
                          </p>
                        </>
                      )}
                    </div>
              ...
              {currentImage && currentImageData && currentImageUrl && (
              ...
              {showChat && (
                <div className="flex flex-col flex-1 h-full">
                  <textarea
                    ...
                    placeholder="我是一个由OpenAI开发的人工智能助手，用于回答问题、提供信息和帮助完成各种任务。你可以问我任何问题或请求帮助，我会尽力提供有用的信息或指导。需要帮助吗？"
                    value={chat}
                    disabled
                  />
                </div>
